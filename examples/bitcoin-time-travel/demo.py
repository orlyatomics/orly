...
# --- Engine ops (over the shared `orly` client) ---------------------------

def credit_at(c, pov, branch, addr, amount, h):
    c.call(pov, "bitcoin", "credit_at",
           {"branch": branch, "addr": addr, "amount": amount, "h": h})


def balance_at(c, pov, branch, addr, h):
    return c.call(pov, "bitcoin", "balance_at",
                  {"branch": branch, "addr": addr, "h": h})


def fork_from(c, pov, branch, parent, fork_h):
    c.call(pov, "bitcoin", "fork_from",
           {"branch": branch, "parent": parent, "fork_h": fork_h})
...
def print_chain(c, pov, branch, max_h):
    header = "  h    " + "  ".join(f"{a:>7}" for a in WATCH)
    print(header)
    print("  " + "-" * (len(header) - 2))
    for h in range(0, max_h + 1):
        bals = [balance_at(c, pov, branch, addr, h) for addr in WATCH]
...
def main():
    ...
    for h, ops in enumerate(MAINNET_BLOCKS, start=1):
        for addr, delta in ops:
            credit_at(c, pov, "mainnet", addr, delta, h)

    print(f"forking at h={FORK_H}, applying {len(FORK_BLOCKS)} fork blocks...")
    fork_from(c, pov, "fork", "mainnet", FORK_H)
    for i, ops in enumerate(FORK_BLOCKS):
        h = FORK_H + i
        for addr, delta in ops:
            credit_at(c, pov, "fork", addr, delta, h)

    total_h = len(MAINNET_BLOCKS)

    print("\n=== MAINNET ===")
    print_chain(c, pov, "mainnet", total_h)

    print(f"\n=== FORK (forked from mainnet at h={FORK_H}) ===")
    print_chain(c, pov, "fork", total_h)

    print("\n=== mainnet vs fork, per height (* = post-fork) ===")
    print("  h     mainnet alice  fork alice    mainnet bob   fork bob")
    print("  " + "-" * 58)
    for h in range(0, total_h + 1):
        ma = balance_at(c, pov, "mainnet", "alice", h)
        fa = balance_at(c, pov, "fork", "alice", h)
        mb = balance_at(c, pov, "mainnet", "bob", h)
        fb = balance_at(c, pov, "fork", "bob", h)
    ...
    for h, expected_btc in EXPECT_MAINNET:
        for addr, want_btc in zip(WATCH, expected_btc):
            got = balance_at(c, pov, "mainnet", addr, h)
    ...
    for h, expected_btc in EXPECT_FORK:
        for addr, want_btc in zip(WATCH, expected_btc):
            got = balance_at(c, pov, "fork", addr, h)
            if got != want_btc * SAT_PER_BTC:
