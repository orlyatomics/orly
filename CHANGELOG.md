...
### Added
- **Go client library** (`clients/go`, package `orly`). The Go counterpart of the Python client for the WS+JSON protocol — `Connect`/session lifecycle, `Call(pov, pkg, method, args)`, and a `Lit` orlyscript-literal encoder (`map[string]any`→record, `[]any`→list, `orly.Set`→`{…}`, escaped strings, `orly.Raw` passthrough). Demos pull it via a local `replace` directive. First consumer: the `recursive-variants` Go driver ported onto it and verified end-to-end against `orlyi`; remaining 6 Go drivers to follow. (2026-06-23)
