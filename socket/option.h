/* <socket/option.h>
   ...
   Constant instances of the standard socket objects appear at the bottom of this header.
   Use these objects when you want to set options on your sockets:

       int my_sock = socket(...);
       Socket::KeepAlive.Set(my_sock, true);  // Turn the 'keep_alive' option on.
   ...
   You can also use these objects to dump the value of the option for your socket
...
namespace Socket {
  ...
  /* This is an intentionally incomplete type we use to specify explicit specializations.
     It represents a string argument of the given maximum size.
  ...
  /* This is a look-up template used to convert between the low-level types expected by the
     OS and the types we would prefer to use in C++.  Explicit specializations exist for the
     following mappings:
    ...
    NOTE: There are times when the OS intends an int to be interpreted as a bool.
    This is handled on a per-case basis in the instantiations of the option objects.

    Each specialization contains a definition of GetSockOpt() and SetSockOpt() which take the
    appropriate type.  The specialization for Str<MaxSize> is slightly different in that it its
    functions take std::string but limit the string to the requested size. */
  ...
  class TAnyOption {
    ...
    /* The code number used by getsockopt() and setsockopt() when referring to this option. */
    int GetCode() const {
      return Code;
    }
    ...
    /* Get the option's value from the given socket and write to the given stream
       in a human-readable form, like this: "<option name>: <val>". */
  ...
  /* The base class for read-only and read-write socket options of a particular type.
     The selector argument is used to pick which Conv<> overload to use when getting
     and setting the option.  The Conv<> overload, in turn, picks the C++ type used
     by the option. */
  template <typename TSelector>
  class TOption
      : public TAnyOption {
  ...
  template <typename TSelector>
  class TRoOption final
      : public TOption<TSelector> {
    public:
  ...
  /* Pointers to all the currently defined socket options, terminated by a null pointer.
     NOTE: BindToDevice, RcvBufForce, and SndBufForce are omitted from this collection because they require
     root privilege, and PeerCred is omitted from this collection because it is only defined for AF_UNIX sockets. */
  extern const TAnyOption *AllOptions[];

  /* For each option in AllOptions, get the option's value from the given socket and write to the given stream
     in a human-readable form.  The result looks like this: "{ <option name>: <val>, <option name>: <val>, ... }". */
  void DumpAllOptions(std::ostream &strm, int sock);

  /* Just like DumpAllOptions(), but the output is sent to the system logger at the given logging level. */
  void LogAllOptions(int log_level, int sock);

  /**
   *  The standard socket options.  These are drawn from the socket(7) man page, q.v, and appear here in the same
  ...
  /* Bind this socket to a particular device like "eth0", as specified in the passed interface name. If the name is an
     ...
     length null-terminated interface name string with the maximum size of IFNAMSIZ. If a socket is bound to an interface,
     only packets received from that particular interface are processed by the socket. Note that this only works for some
     socket types, particularly AF_INET sockets. It is not supported for packet sockets (use normal bind(2) there). */
  extern const TRwOption<Str<IFNAMSIZ>> BindToDevice;

  /* Set or get the broadcast flag. When enabled, datagram sockets receive packets sent to a broadcast address and they
     are allowed to send packets to a broadcast address. This option has no effect on stream-oriented sockets. */
  ...
  /* Enable BSD bug-to-bug compatibility. This is used by the UDP protocol module in Linux 2.0 and 2.2. If enabled ICMP
     ...
     program uses this option. Linux 2.0 also enabled BSD bug-to-bug compatibility options (random header changing, skipping
  ...
  /* When enabled, a close(2) or shutdown(2) will not return until all queued messages for the socket have been successfully
     sent or the linger timeout has been reached. Otherwise, the call returns immediately and the closing is done in the
     background. When the socket is closed as part of exit(2), it always lingers in the background. */
  extern const TRwOption<TLinger> Linger;

  /* If this option is enabled, out-of-band data is directly placed into the receive data stream. Otherwise out-of-band data
     is only passed when the MSG_OOB flag is set during receiving. */
  ...
  /* Return the credentials of the foreign process connected to this socket. This is only possible for connected
     AF_UNIX stream sockets and AF_UNIX stream and datagram socket pairs created using socketpair(2); see unix(7).
  ...
  /* Set the protocol-defined priority for all packets to be sent on this socket. Linux uses this value to order the
     ...
     discipline. For ip(7), this also sets the IP type-of-service (TOS) field for outgoing packets. Setting a priority outside
  ...
  /* Sets or gets the maximum socket receive buffer in bytes. The kernel doubles this value (to allow space for bookkeeping
     overhead) when it is set using setsockopt(2), and this doubled value is returned by getsockopt(2). The default value is
     set by the /proc/sys/net/core/rmem_default file, and the maximum allowed value is set by the /proc/sys/net/core/rmem_max
     file. The minimum (doubled) value for this option is 256. */
  extern const TRwOption<int> RcvBuf;

  /* Using this socket option, a privileged (CAP_NET_ADMIN) process can perform the same task as SO_RCVBUF, but the rmem_max
  ...
  /* Specify the minimum number of bytes in the buffer until the socket layer will pass the data to the protocol (SO_SNDLOWAT)
     or the user on receiving (SO_RCVLOWAT). These two values are initialized to 1. SO_SNDLOWAT is not changeable on Linux
     (setsockopt(2) fails with the error ENOPROTOOPT). SO_RCVLOWAT is changeable only since Linux 2.4. The select(2) and poll(2)
     system calls currently do not respect the SO_RCVLOWAT setting on Linux, and mark a socket readable when even a single byte
  ...
  /* Specify the receiving or sending timeouts until reporting an error. The argument is a struct timeval. If an input or output
     function blocks for this period of time, and data has been sent or received, the return value of that function will be the
     amount of data transferred; if no data has been transferred and the timeout has been reached then -1 is returned with errno
     set to EAGAIN or EWOULDBLOCK just as if the socket was specified to be nonblocking. If the timeout is set to zero (the default)
     then the operation will never timeout. Timeouts only have effect for system calls that perform socket I/O (e.g., read(2),
     recvmsg(2), send(2), sendmsg(2)); timeouts have no effect for select(2), poll(2), epoll_wait(2), etc. */
  ...
  /* Indicates that the rules used in validating addresses supplied in a bind(2) call should allow reuse of local addresses.
     ...
     When the listening socket is bound to INADDR_ANY with a specific port then it is not possible to bind to this port for any
     local address. */
  ...
  /* Enable or disable the receiving of the SO_TIMESTAMP control message. The timestamp control message is sent with level SOL_SOCKET
     and the cmsg_data field is a struct timeval indicating the reception time of the last packet passed to the user in this call.
