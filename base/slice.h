...
namespace Base {
  ...
  /* A value type for specifying how to slice a sequence.  The slice extends from its start position up to (but not including) its limit.  The endpoints are
     instances of TPos, which means you can specify them relative to the start or end of the sequence.

     Because TPos can construct from an integer, you can generally specify slices like this: TSlice(0, 10).  (This would be the slice extending from element #0
     up to but not including element #10; in other words, the first 10 elements in the sequence.)  This should match the intuition you developed when working
     with arrays.

     You can also use the constants defined in TPos to define slices.  For example, TSlice(10, TPos::Limit) means 'the slice starting at element #10 and
     continuing through the rest of the sequence'.  Notice that, due to the defaults provided in the TSlice constructor, you can say the same thing by just
     saying TSlice(10).

     You can also construct explicit instances of TPos to get specific slices.  For example, TSlice(TPos(10, TPos::Reverse), TPos(7, TPos::Reverse)) means 'the
     slice starting 10 elements back from the end and continuing for the next 3 elements'. */
  class TSlice {
    public:
    ...
    bool TryGetAbsPair(size_t size, size_t &start, size_t &limit) const {
      ...
      bool success =
        (start_offset >= 0            && start_offset <= static_cast<int>(size) &&
         limit_offset >= start_offset && limit_offset <= static_cast<int>(size));
      if (success) {
        start = start_offset;
        limit = limit_offset;
      }
    ...
    /* The start and limit (relative) positions of the slice.  These are directly accessible and freely modifiable. */
    TPos Start, Limit;

    /* We define three global slices for convenience.  Each represents some sort of degenerate case.
