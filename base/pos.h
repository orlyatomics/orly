/* <base/pos.h>
   ...
   A position in a sequence, relative to ether the start or limit of the sequence.
...
namespace Base {
  ...
  /* A value type for specifing a position in a sequence, relative to either the start or limit of the sequence.  Our position is represented as a signed
     offset into the sequence, allowing us to refer to positions that are outside of the sequence proper. */
  class TPos {
    public:
    ...
    enum TDir {
      ...
      /* Our direction is 'forward', meaning from the start and toward the end of the sequence.  Our offset will therefore be interpreted thus:

             -1        the first element before the start of the sequence  (ReverseLimit)
         ...
         This is by far the more common way of thinking about a sequence and is, therefore, the default direction for a TPos. */
      Forward,

      /* Our direction is 'reverse', meaning from the end and toward the start of the sequence.  Our offset will therefore be interpreted thus:
             ...
             size      the first element before the start of the sequence  (ReverseLimit)

         This point of view is less common than the other so, when you want it, you must request it specifically. */
      Reverse
    };

    /* Initializing constructor, fully defaultable. */
    TPos(ptrdiff_t offset = 0, TDir dir = Forward) : Offset(offset), Dir(dir) {}
    ...
    /* Our offset, relative to our direction.  This member is public because we enforce no rules about it. */
    ptrdiff_t Offset;

    /* Our direction.  This member is public because we enforce no rules about it. */
    TDir Dir;
