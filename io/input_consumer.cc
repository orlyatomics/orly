...
void TInputConsumer::TryRefresh() {
  ...
  if (Cursor >= Limit) {
    /* If we have a current chunk, we're now done with it.
       If we're collecting chunks (because of one or more marks), then move our index forward;
    ...
    /* Get our new current chunk.
       If we've not yet reached the end of our cache (because we rewound and are moving forward again), just take the next cached chunk;
       otherwise, try to get another chunk from our producer. */
...
TMark::~TMark() {
  ...
  if (!OlderMark) {
    /* We were the last mark, so it's time to clean up the collection of chunks.
       Release all the chunks before the consumer's current chunk, making the current chunk the first in the container.
       If it has no current chunk, we'll release all the chunks. */
