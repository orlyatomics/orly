...
# image ships (jhm emits to /src/out_orly).
#
# The version has to be passed IN. `make version` derives it from `git
# describe`, and .dockerignore excludes .git -- deliberately, it is build
# context nothing here needs -- so inside this build it always degraded to
# "unknown". That was tolerable while the image was only ever :latest; it is
# not once there are release tags, because `orlyc --help` reporting
# "Build: unknown" leaves a user with no way to ask the binary what it is.
# The workflow passes the tag on a release and dev-<short sha> otherwise.
ARG JHM_WORKER_COUNT=4
ARG ORLY_VERSION=
RUN if [ -n "$ORLY_VERSION" ]; then \
      printf '#pragma once\n\n/* Generated by the image build; see Dockerfile. */\n#define ORLY_BUILD_VERSION "%s"\n' \
        "$ORLY_VERSION" > orly/build_version.h; \
      make bootstrap; \
    else \
