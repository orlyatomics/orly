/* <orly/server/cold_start_test.cc>
...
FIXTURE(SmallTypical) {
  ...
  out_path << "release";
  #endif
  out_path << "/orly/server/";
  /* make the path to the service argument ini file */
  /* make the path to the server binary */
  stringstream ini_path;
  ini_path << "--args=" << SRC_ROOT << "/orly/server/cold_start_test.ini";

  stringstream path;
  path << out_path.str() << "orly";
  ...
  /* launch the server which will start as solo and create a new disk image. */ {
    ...
    /* install the package on the server */ {
      auto client = make_shared<TExerciseClient>(*std::optional<TUuid>::Unknown, server_addr);
    ...
    /* connect a client to the server and push a key */ {
      ...
      auto pov_id = client->NewFastPrivatePov(*std::optional<TUuid>::Unknown, seconds(10));
