/* <orly/rt/time_diff_obj.h>

   The `TObj` record returned by orly's `TimeDiff` builtin --
   `{day, hour, is_forward, minute, nanosecond, second}`. Mostly
   auto-generated by orlyc itself (see the in-file comment) and
   checked into the runtime so code-gen has the type available
   before any user package is loaded. The only hand-edited part is
   the constructor that builds the record from
   `Base::Chrono::TTimeDiffInfo`.
  ...
  namespace Type {
    ...
    struct TDt<Rt::Objects::TObjO6i3dayi4hourb10is_forwardi6minutei10nanosecondi6second> {
      ...
      static TType GetType() {
        return TObj::Get({{"day", Orly::Type::TInt::Get()}, {"hour", Orly::Type::TInt::Get()}, {"is_forward", Orly::Type::TBool::Get()}, {"minute", Orly::Type::TInt::Get()}, {"nanosecond", Orly::Type::TInt::Get()}, {"second", Orly::Type::TInt::Get()}});
      }
...
namespace std {
  template<>
  struct hash<Orly::Rt::Objects::TObjO6i3dayi4hourb10is_forwardi6minutei10nanosecondi6second> {

    typedef size_t return_type;
    typedef Orly::Rt::Objects::TObjO6i3dayi4hourb10is_forwardi6minutei10nanosecondi6second argument_type;

    size_t operator()(const argument_type &obj) const {
      return  obj.GetHash();
    }

  }; // hash<Orly::Rt::Objects::TObjO6i3dayi4hourb10is_forwardi6minutei10nanosecondi6second>
} // std