...
TLanguage *Tools::Nycr::Symbol::Bootstrap() {
  ...
  new TKeyword("left_kwd", assoc, "left", std::nullopt);
  new TKeyword("right_kwd", assoc, "right", std::nullopt);
  new TKeyword("nonassoc_kwd", assoc, "nonassoc", std::nullopt);
  ...
  // rr_kwd    = 'rr';
  // sr_kwd    = 'sr';
  auto empty_kwd = new TKeyword("empty_kwd", 0, "empty", std::nullopt);
  auto error_kwd = new TKeyword("error_kwd", 0, "error", std::nullopt);
  auto prec_kwd = new TKeyword("prec_kwd", 0, "prec", std::nullopt);
  auto pri_kwd = new TKeyword("pri_kwd", 0, "pri", std::nullopt);
  auto rr_kwd = new TKeyword("rr_kwd", 0, "rr", std::nullopt);
  auto sr_kwd = new TKeyword("sr_kwd", 0, "sr", std::nullopt);
  ...
  // semi        = '";"';
  // slash       = '"/"';
  auto arrow = new TKeyword("arrow", 0, "\\-\\>", std::nullopt);
  auto colon = new TKeyword("colon", 0, "\\:", std::nullopt);
  auto close_angle = new TKeyword("close_angle", 0, "\\>", std::nullopt);
  auto eq = new TKeyword("eq", 0, "\\=", std::nullopt);
  auto open_angle = new TKeyword("open_angle", 0, "\\<", std::nullopt);
  auto semi = new TKeyword("semi", 0, "\\;", std::nullopt);
  auto slash = new TKeyword("slash", 0, "\\/", std::nullopt);

  // int_literal = r'(\+|\-)?[[:digit:]]+';
  auto int_literal = new TKeyword("int_literal", 0, "(\\+|\\-)?[[:digit:]]+", std::nullopt);
  ...
  new TKeyword("single_quoted_str_literal", str_literal, "{SINGLE_QUOTED_STRING}", std::nullopt);
  new TKeyword("double_quoted_str_literal", str_literal, "{DOUBLE_QUOTED_STRING}", std::nullopt);
  ...
  auto nycr = new TLanguage("nycr", 0, { "tools", "nycr", "syntax" }, std::nullopt, std::nullopt);
